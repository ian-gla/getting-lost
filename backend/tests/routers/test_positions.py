...
def test_create_missing_attr(app_client: TestClient):
    payload = {
        "start": "POINT(55.872179 -4.292532)",
        "end": "POINT(55.8723 -4.289259)",
        "start_radius": "100",
...
def test_update(app_client: TestClient, create_position: Positions):
    ...
    assert rv.status_code == 200, rv.json()
    response = rv.json()
    assert response["start"] == "POINT (50 -4)"


def test_update_bad_id(app_client: TestClient, create_position: Positions):
